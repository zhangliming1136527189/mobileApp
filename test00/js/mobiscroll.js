(function($) {
    function Scroller(elem, settings) {
        ...
        // Private functions
        function giveDate() {
            var seasonlist = ['一季度', '二季度', '三季度', '四季度'];
            var dateBegin;
            var dateEnd;
            var yearObjOne;
            var yearObjTwo;
            ...
            var daymilliseconds;
            var days;
            if (sessionStorage.getItem('Ykey') == '1') {
                var getDate = $('.date-begin').val();
                $('.date-end').val(getDate);
                var getDate_1 = getDate.split('-')[1];
                var date = Math.ceil(getDate_1 / 3 - 1);
                if (date <= 0) {
                    $('.my-season').html(seasonlist[0])
                } else {
                    $('.my-season').html(seasonlist[date])
                }
                dateBegin = $('.date-begin').val();
                dateEnd = $('.date-end').val();
                yearObjOne = new Date(dateBegin);
                yearObjTwo = new Date(dateEnd);
                milliseconds = yearObjTwo.getTime() - yearObjOne.getTime();
                daymilliseconds = 3600 * 24 * 1000;
                days = milliseconds / daymilliseconds;
                if (days >= 0) {
                    $('.my-dataNum').html(days);
                }
            } else if (sessionStorage.getItem('Ykey') == '2') {
                dateBegin = $('.date-begin').val();
                dateEnd = $('.date-end').val();
                yearObjOne = new Date(dateBegin);
                yearObjTwo = new Date(dateEnd);
